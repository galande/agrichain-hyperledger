   ...
   */
    ...
    */
     ...
     */
     ...
     /**
      * This EnterTempRatureReadingToShipment transaction will add temprature reading to shipement - Shipper
      * @param {org.agrichain.transaction.TemperatureReading} shipmentInfo
      * @transaction
      */

      function temperatureReading(shipmentInfo) {
          
        const factory = getFactory();
        const assetNs = 'org.agrichain.asset';
        const transactionNs = 'org.agrichain.transaction';
        let shipmentRegistry = {};
        ...
        return getAssetRegistry(assetNs + '.Shipement').then(function (registry) {
            ...
            if (shipement.temperatureReadings){
                shipement.temperatureReadings.push(shipmentInfo);
                console.log('Push called.');
            }else{
                shipement.temperatureReadings = [shipmentInfo];
                console.log("Normal called");
                
            }

            let temperatureReadingEvent = factory.newEvent(transactionNs, 'AddedTemperatureReadingToShipemt');
